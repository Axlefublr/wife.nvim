...
---@field prompt string|boolean|nil
---Path to the shell you want `interactive_shell` and `interactive_async_shell` to use.
...
---Prompt for `require('wife').interactive_async_shell()`
---The two are separated mostly because `dress.nvim` / `dressing.nvim`
---would put the prompt in the title of a floating window, and it doesn't make sense
---to be there, and so you should be able to configure it separately.
---(You don't have to use either of those plugins)
---@field async_prompt string|boolean|nil
...
function m.setup(opts)
	...
	if type(plugin_opts.prompt) == 'boolean' then plugin_opts.prompt = nil end
	if type(plugin_opts.async_prompt) == 'boolean' then plugin_opts.async_prompt = nil end
end
...
function m.display(output, only_errors)
	...
	if #output_lines == 1 then
		vim.notify(unpack(output_lines), successful and vim.log.levels.OFF or vim.log.levels.ERROR)
		return
	end
...
---Take a `vim.SystemCompleted`,
---(is an argument in `vim.system`'s on_exit function, and is returned after `:wait()`ing a `vim.system` call)
---and diplay it in `:messages`
---@param output vim.SystemCompleted
---@param only_errors boolean? Only show output if the exitcode is not 0.
function m.display_in_messages(output, only_errors)
	local successful = output.code == 0
	if only_errors and successful then return end
	print(output.stdout)
	print(output.stderr)
...
---Do `require('wife').shell()` asyncronously,
---and display the output in `:messages`
---@param cmd string|string[] Can be string if all you want to run is a single command with no arguments.
---@param only_errors boolean? Only show output if the exitcode is not 0.
---@param opts vim.SystemOpts?
function m.shell_async_display(cmd, only_errors, opts)
	m.shell(cmd, opts, function(output) m.display_in_messages(output, only_errors) end)
end
...
---Enter a shell command using `vim.ui.input` and `require('wife').shell_async_display()` it.
---If the first character is `;` (configurable with the `errorer` option), `only_errors` is set to `true`.
function m.interactive_async_shell()
	vim.ui.input({ prompt = plugin_opts.async_prompt }, function(input)
		if not input then return end
		local only_errors = false
		if input:sub(1, #plugin_opts.errorer) == plugin_opts.errorer then
			input = input:sub(#plugin_opts.errorer + 1)
		...
		local args = { plugin_opts.shell, '-c', input }
		m.shell_async_display(args, only_errors)
	end)
end
